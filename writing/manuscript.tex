...
The key advantage of this approach is that by conditioning on each referrer's enrollment network, we eliminate selection bias from program choice and other factors that determine who appears in each person's choice set. The identifying variation comes from within-network differences in referral decisions, holding constant the pool of available candidates.

We estimate separate models for each referrer SES group to estimate aggregate SES biases across socioeconomic groups. 
