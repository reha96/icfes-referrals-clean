...
twoway (bar prop_to_low xpos, barw(0.6) fcolor(gs11) lcolor(gs4)) ///
       ...
       xtitle("") ///
	   title("Low-SES", size(medlarge)) ///
       legend(order(2 "University share") ring(0) pos(2) region(lcolor(none)) size(small)) ///
       graphregion(color(white)) bgcolor(white) ///
       xscale(range(0.5 3.5)) ///
       name(to_low_ses, replace) nodraw
...
twoway (bar prop_to_middle xpos, barw(0.6) fcolor(gs11) lcolor(gs4)) ///
       ...
       xtitle("") ///
	   ytitle("Proportion", size(medium)) ///
       title("Middle-SES", size(medlarge)) ///
       legend(order(2 "University share") ring(0) pos(2) region(lcolor(none)) size(small)) ///
       graphregion(color(white)) bgcolor(white) ///
       xscale(range(0.5 3.5)) ///
       name(to_middle_ses, replace) nodraw
...
twoway (bar prop_to_high xpos, barw(0.6) fcolor(gs11) lcolor(gs4)) ///
       ...
       ytitle("Proportion", size(medium)) ///
	   xtitle("") ///
       title("High-SES", size(medlarge)) ///
       legend(order(2 "University share") ring(0) pos(2) region(lcolor(none)) size(small)) ///
       graphregion(color(white)) bgcolor(white) ///
       xscale(range(0.5 3.5)) ///
       name(to_high_ses, replace) nodraw

// Combine all three plots
graph combine to_low_ses to_middle_ses to_high_ses, ///
    rows(1) cols(3) ///
    title("Network Composition by SES", size(large))  ///
