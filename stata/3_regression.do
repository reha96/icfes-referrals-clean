...
// eststo reg2: clogit nomination ib(2).other_estrato z_tie ib(0).same_program, group(area_id) vce(cluster own_id)
// esttab reg*, cells(b(star fmt(3)) se(par fmt(3))) star(* 0.10 ** 0.05 *** 0.01) scalars("N Obs." "N_clust Ind." "chi2 Chi-test") sfmt(0 0 2) nodep nomti label ty 
...
forvalues ses = 1/3 {
    preserve		
		...
		test 1.other_estrato = 2.other_estrato // *** only for low-SES
	restore
}
