...
gen delta_ownXlses = delta_own_belief * own_low_ses
gen delta_otherXlses = delta_other_belief * own_low_ses
eststo d1: reg score_premium own_score  delta_own_belief delta_other_belief
eststo d2: reg score_premium own_score  delta_own_belief delta_other_belief i.treat  
eststo d3: reg score_premium own_score  delta_own_belief delta_other_belief tie sd_other_score i.area i.treat ib(2).own_estrato  
eststo d4: reg score_premium own_score  delta_other_belief delta_own_belief own_low_ses delta_ownXlses delta_otherXlses
cls
...
twoway (line delta_own_margins1 delta_own_at1, lcolor("136 132 216") lwidth(thick)) ///
       ...
       ytitle("Linear Prediction") ///
       xtitle("Δ Belief (positive = overestimation)") ///
       xlabel(-25(5)25) ///
...
twoway (histogram delta_other_belief, percent fcolor(gs10) bins(20) lcolor(gs4) lwidth(thin)) ///
      ...
      ytitle("Percent") ///
      xtitle("") ///
      title("Δ Nominee Belief (positive = overestimation)") ///
      graphregion(color(white)) bgcolor(white) ///
