...
forvalues t = 1/30 {
   ...
   local prop_sameLOW = cond(stat[5,2] != ., stat[5,2], 0)
   
   quietly proportion same_ses if own_estrato == 1
   matrix stat = r(table)
   local prop_lses = cond(stat[1,2] != ., stat[1,2], 1)
   
   quietly proportion same_ses if own_estrato == 2
   matrix stat = r(table)
   local prop_mses = cond(stat[1,2] != ., stat[1,2], 0)
   
   quietly proportion same_ses if own_estrato == 3
   matrix stat = r(table)
   local prop_hses = cond(stat[1,2] != ., stat[1,2], 0)
