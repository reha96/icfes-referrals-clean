/*******************************************************************************
    ...
    Author: Reha Tuncer
    Date: 18.03.2025
    Description: figure tie strength by SES
*******************************************************************************/
...
ttest tie if (own_estrato==1 | own_estrato==2) & other_estrato==1, by(own_estrato)
ttest tie if (own_estrato==1 | own_estrato==3) & other_estrato==1, by(own_estrato)
ttest tie if (own_estrato==2 | own_estrato==3) & other_estrato==1, by(own_estrato)

// Compare Low-Middle vs Middle-Middle vs High-Middle (connections to Middle SES peers)
ttest tie if (own_estrato==1 | own_estrato==2) & other_estrato==2, by(own_estrato)
ttest tie if (own_estrato==1 | own_estrato==3) & other_estrato==2, by(own_estrato)
ttest tie if (own_estrato==2 | own_estrato==3) & other_estrato==2, by(own_estrato)

// Compare Low-High vs Middle-High vs High-High (connections to High SES peers)
ttest tie if (own_estrato==1 | own_estrato==2) & other_estrato==3, by(own_estrato)
ttest tie if (own_estrato==1 | own_estrato==3) & other_estrato==3, by(own_estrato)
ttest tie if (own_estrato==2 | own_estrato==3) & other_estrato==3, by(own_estrato)
...
twoway (bar tie_strength xpos if other_ses == 1, barw(0.25) color("255 99 132")) ///
       ...
       ylabel(0(1)6, angle(0) format(%9.0f)) ///
...
graph export "/Users/reha.tuncer/Documents/GitHub/icfes-referrals/figures/tie_strength.png", replace
restore
