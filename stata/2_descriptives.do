...
//# IIR assumption: are same SES individuals substitutes?
use "${dpath}dataset_z.dta", clear
keep if nomination 
keep own_id other_id other_estrato 

bysort own_id : gen c = _n
bysort own_id : egen _t = max(c)
bysort own_id : replace c  = _t
drop _t
keep if c == 2

bysort own_id : gen i = _n
reshape wide other_estrato other_id, i(own_id) j(i)
list in 1/20
keep if other_id1 != other_id2
gen subst = (other_estrato1 == other_estrato2)
sum subst
