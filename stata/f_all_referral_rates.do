...
global prop_low = results[1,1]
global prop_middle = results[1,2]
global prop_high = results[1,3]

global se_low = results[2,1]*100
global se_middle = results[2,2]*100
global se_high = results[2,3]*100
...
proportion own_estrato

matrix results = r(table)
global prop_lowA = results[1,1]
global prop_middleA = results[1,2]
global prop_highA = results[1,3]

global se_lowA = results[2,1]*100
global se_middleA = results[2,2]*100
global se_highA = results[2,3]*100

sum other_estrato
global n2 = r(N)
restore 

prtesti ${n1} ${prop_low} ${n2} ${prop_lowA}
global p_low = r(p)
prtesti ${n1} ${prop_middle} ${n2} ${prop_middleA}
global p_middle = r(p)
prtesti ${n1} ${prop_high} ${n2} ${prop_highA}
global p_high = r(p)

// Create significance indicators
global sig_low = cond(${p_low}<.001, "***", cond(${p_low}<.01, "**", cond(${p_low}<.05, "*", cond(${p_low}<.1, "+", ""))))
global sig_middle = cond(${p_middle}<.001, "***", cond(${p_middle}<.01, "**", cond(${p_middle}<.05, "*", cond(${p_middle}<.1, "+", ""))))
global sig_high = cond(${p_high}<.001, "***", cond(${p_high}<.01, "**", cond(${p_high}<.05, "*", cond(${p_high}<.1, "+", ""))))
...
replace sig = "${sig_low}" if own_estrato == 1
replace sig = "${sig_middle}" if own_estrato == 2
replace sig = "${sig_high}" if own_estrato == 3

gen sig_pos = max(rateN, rateA) + 3
...
twoway (bar rateA xpos, barwidth(0.3) fcolor(gs8) lcolor(gs4)) ///
	   ...
	   (scatter sig_pos own_estrato, mlabsize(large) mlabcolor(dknavy) msymbol(none) mlabel(sig) mlabposition(12)) ///
       , ///
       xlabel(1 "Low" 2 "Middle" 3 "High") ///
       ylabel(0(10)80, angle(0) format(%9.0f) grid gmax gmin) ///
       ytitle("Percent") ///
       xtitle("") ///
       title("Referrals to SES groups") ///
       legend(order(1 "Sample" 3 "Referral") ///
